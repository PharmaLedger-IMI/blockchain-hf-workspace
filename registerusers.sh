...
echo "Creating the identities for the hlf-adapter: rcaadmin"
...
fabric-ca-client register --caname ca-org1 --id.name rcaadmin --id.secret rcaadminpw --id.type admin --id.attrs "hf.Registrar.Roles=""*"",hf.Registrar.DelegateRoles=""*"",hf.Revoker=true,hf.IntermediateCA=true,hf.GenCRL=true,hf.Registrar.Attributes=""*"",hf.AffiliationMgr=true"  --tls.certfiles "${PWD}/organizations/fabric-ca/org1/ca-cert.pem" 
